...
//	move_page_to_inactive();	
	...
	int origin_len = linux_evict_using_page[1];
	int i = 0;
...
//	for(i = 0; i < RECLAIM_BATCH * 2;i++){
	for(i = 0; i < origin_len; i++){
		...
		{
			printf("error, scan too mus active list!\n");
			break;
		}
		if( find_old_page >= TRANSFER_BATCH )
                        break;
	...
	if(find_old_page < TRANSFER_BATCH){
		origin_len = linux_evict_using_page[1];
		tmp = &linux_evict_tail[1];
		tmp = tmp -> prev;
		scan_ptr = tmp;
		for(i = 0; i < origin_len; i++){
...
unsigned long select_and_return_free_ppn_clock(unsigned long vpn){
	...
	int origin_inactive = linux_evict_using_page[0];	
	
	for(i = 0; i < origin_inactive; i++){
